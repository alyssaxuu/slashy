$(document).ready(() => {
	...
	var cursorpos;
	var saving = false;
	const commandbutton = '<div id="notion-command-button"><img src="' + browser.runtime.getURL("assets/zap.svg") + '"></div>';
	...
	function fixReact() {
		...
		s.src = browser.runtime.getURL('force.js');
		s.onload = function() {
				this.remove();
		};
	...
	function injectContent() {
			...
			$.ajax({
					...
					success: function(data) {
							...
							$("#notion-close").attr("src", browser.runtime.getURL("assets/close.svg"));
							$("#notion-close-2").attr("src", browser.runtime.getURL("assets/close.svg"));
							$("#notion-close-3").attr("src", browser.runtime.getURL("assets/close.svg"));
							$("#notion-close-4").attr("src", browser.runtime.getURL("assets/close.svg"));
							$(".notion-form-dropdown-right img").attr("src", browser.runtime.getURL("assets/dropdown.svg"));
							$("#notion-pencil img").attr("src", browser.runtime.getURL("assets/pencil.svg"));
							$("#notion-line img").attr("src", browser.runtime.getURL("assets/line.svg"));
							$("#notion-text img").attr("src", browser.runtime.getURL("assets/text.svg"));
							$("#notion-eraser img").attr("src", browser.runtime.getURL("assets/eraser.svg"));
							$("#notion-clear img").attr("src", browser.runtime.getURL("assets/clear.svg"));
	...
	function renderItems() {
			...
			gettingItem.then((res) => {
					if (typeof res === "undefined") {
							...
							res.commands.forEach(function(command) {
	...
	function renderCommandItem(command) {
			...
			const onerror = 'this.onerror = null; this.src="' + browser.runtime.getURL("assets/fallback.png") + '"';
			if (command.id == 0 || command.id == 1) {
					...
					$(".notion-command-sidebar-body").append("<div class='notion-command-sidebar-item' data-id='" + command.id + "'><div class='notion-command-sidebar-item-icon'><img src='" + img + "' onerror='" + onerror + "'></div><div class='notion-command-sidebar-item-info'><div class='notion-command-sidebar-item-title'>" + name + "</div><div class='notion-command-sidebar-item-desc'>" + desc + "</div></div><div class='notion-command-sidebar-hover'><span>Edit</span><div class='notion-command-item-hover-wrap'><img src='" + browser.runtime.getURL("assets/trash.svg") + "'></div></div></div>")
			}
	}

	function renderPlaceholder() {
			$(".notion-command-sidebar-body").html('<img src="' + browser.runtime.getURL("assets/cooltest.svg") + '" id="command-placeholder">');
	}
	...
	function createModal(title, label, fun) {
			document.body.innerHTML += '<div class="notion-popup" id="notion-plugin-popup"><div class="notion-head"><div class="notion-title">' + title + '</div><img src="' + browser.runtime.getURL("assets/close.svg") + '" id="notion-close-3"></div><div id="notion-plugin-popup-label">' + label + '</div><input class="notion-form-input" id="notion-plugin-popup-input" placeholder="Something here"><div id="notion-plugin-popup-submit">Submit</div></div>';
	...
	function checkForm() {
			...
			const name = sanitize($("input[name='notion-form-command-name']").val());
			const description = sanitize($("input[name='notion-form-command-description']").val());
			const command = $("input[name='notion-form-command'").val();
			const content = $(".notion-form-textarea").val();
			const type = sanitize($("#notion-form-type").attr("data-id"));
			const image = sanitize($("#notion-image-field").val());
			const visibility = sanitize($("#notion-form-visibility").attr("data-id"));
			var flagcheck = false;
	...
	function saveForm(name, description, command, content, type, visibility, image) {
			...
			gettingItem.then((num) => {
					if (typeof num === "undefined") {
							console.log("ERROR");
					} else {
							var gettingItem2 = browser.storage.sync.get('commands');
							gettingItem2.then((result) => {
			...
			gettingItem.then((result) => {
					if (typeof result === "undefined") {
							...
							result.commands.forEach(function(command) {
									if (command.image == "") {
											command.image = browser.runtime.getURL("assets/tempthing.png")
									}
							});
